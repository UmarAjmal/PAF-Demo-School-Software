...
async function runStep(stepName, queryFn) {
    try {
        console.log(`   → ${stepName}...`);
        await queryFn();
    } catch (err) {
        console.warn(`   ⚠️ Warning in ${stepName}:`, err.message);
    }
}
...
async function runEssentialMigrations() {
    ...
    try {
        ...
        await runStep("Checking academic_terms columns", async () => {
            await client.query(`
                DO $$
                BEGIN
        ...
        await runStep("Checking monthly_fee_slips columns", async () => {
            await client.query(`
                DO $$
                BEGIN
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='monthly_fee_slips') THEN
        ...
        // 4. School Settings logo_url Migration
        await runStep("Checking school_settings logo_url column type", async () => {
            await client.query(`
                DO $$
                BEGIN
        ...
        // 4.1 Admission Fee Ledger & Payment Tables Migration
        await runStep("Checking admission_fee_ledger & payment tables columns", async () => {
            await client.query(`
                ...
                BEGIN
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='fee_payments') THEN
                        ...
                        END IF;
                    END IF;
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='family_opb_payments') THEN
                        ...
                        END IF;
                    END IF;
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='admission_fee_payments') THEN
        ...
        await runStep("Checking expense_categories and expenses columns", async () => {
            await client.query(`
                DO $$
                BEGIN
        ...
        // 4.3 App Roles & Dashboard Access Migration (Ensures columns exist BEFORE any updates)
        await runStep("Checking app_roles, role columns, and user role assignments", async () => {
            await client.query(`
                CREATE TABLE IF NOT EXISTS app_roles (
                    id SERIAL PRIMARY KEY,
                    role_name VARCHAR(100) UNIQUE NOT NULL,
                    description TEXT,
                    role_level INT DEFAULT 50,
                ...
                ALTER TABLE app_roles ADD COLUMN IF NOT EXISTS role_level INT DEFAULT 50;
                ...
                ALTER TABLE app_roles ADD COLUMN IF NOT EXISTS is_custom BOOLEAN DEFAULT TRUE;

                -- Update system roles to correct default dashboards
                UPDATE app_roles 
        ...
        // 4.4 Fee Heads Arrears, Line Items & WebAuthn Migration
        await runStep("Checking fee_heads, slip_line_items, and webauthn credentials", async () => {
            await client.query(`
                DO $$
                BEGIN
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='fee_heads') THEN
                    ...
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='slip_line_items') THEN
                    ...
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='fee_plan_heads') THEN
        ...
        await runStep("Checking student_academic_records table", async () => {
            await client.query(`
        ...
        // 5.1 Repair incorrectly marked blood siblings
        await runStep("Repairing incorrectly marked blood siblings", async () => {
            await client.query(`
                DO $$
                BEGIN
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='student_siblings') 
                       AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='students') THEN
        ...
        // 6. Exam Marks & Test Papers Approval Workflow Migration
        await runStep("Checking exam_marks & test_papers approval columns and exam_sheet_approvals table", async () => {
            await client.query(`
                DO $$
                BEGIN
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='exam_marks') THEN
                    ...
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='test_papers') THEN
                ...
                DO $$
                BEGIN
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='exam_fee_collections') THEN
        ...
        // 7. User Sessions & Login Security Migration
        await runStep("Checking user_sessions & security columns", async () => {
            await client.query(`
                DO $$
                BEGIN
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='app_users') THEN
        ...
        // 8. Attendance Settings, Holidays & Coordinator Assignments Migration
        await runStep("Checking attendance_settings, holidays & coordinator assignments", async () => {
            await client.query(`
        ...
        // 9. Family Fee & Sibling Monthly Fee Auto-Sync Migration
        await runStep("Checking and synchronizing family members monthly fees", async () => {
            await client.query(`
                DO $$
                BEGIN
                    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='students') 
                       AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='families') THEN
                        UPDATE students s
        ...
        // 10. Synchronize all PostgreSQL sequences
        await runStep("Synchronizing primary key sequences", async () => {
